  ...
  const ngoPlans = [
    {
      ...
      features: (p.ngoFreeFeatures || [
        ...
        "Browse candidate (impact agent) profiles",
        "View paid candidate (impact agent) profiles",
        "Basic candidate (impact agent) matching",
        "Email support",
      ]).map((f: string) => f.replace("{count}", String(platformSettings?.ngoFreeProjectsPerMonth || 3))),
      limitations: p.ngoFreeLimitations || [
        "Cannot unlock FREE candidate (impact agent) profiles",
        "Upgrade to Pro to unlock candidates (impact agents)",
      ],
      popular: false,
    },
    {
      id: "ngo-pro",
      name: p.pro || "Pro",
      description: p.ngoProDesc || "Unlock unlimited FREE candidate (impact agent) profiles",
      ...
      features: platformSettings?.ngoProFeatures || p.ngoProFeaturesDefault || [
        "Unlimited jobs",
        "Unlock UNLIMITED free candidate (impact agent) profiles",
        "View all paid candidate (impact agent) profiles",
