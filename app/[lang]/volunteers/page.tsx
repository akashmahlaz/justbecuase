...
export default function VolunteersPage({ embed, subscriptionPlan }: VolunteersPageProps = {}) {
  ...
  const FilterContent = () => (
    <div className="space-y-6">
      {/* Volunteer Type */}
      <div>
        <Label className="text-sm font-semibold text-foreground mb-3 block">{dict.volunteersListing?.impactAgentType || "Candidate (Impact Agent) Type"}</Label>
        <RadioGroup
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="type-all" className="text-sm font-normal cursor-pointer">
              {dict.volunteersListing?.allImpactAgents || "All Candidates (Impact Agents)"}
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <div className="container mx-auto px-4 md:px-6">
              <div className="max-w-3xl mx-auto text-center">
                <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
                  {dict.volunteersListing?.title || "Find Skilled Candidates (Impact Agents)"}
                </h1>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-muted-foreground mb-4">
                {(dict.volunteersListing?.showingTemplate || "Showing {shown} of {total} candidates (impact agents)")
              ...
              {loading ? (
                ...
                <AIEmptyState
                  mode="empty"
                  title={dict.volunteersListing?.noAgentsFound || "No candidates (impact agents) found"}
                  description={
                    hasActiveFilters || searchQuery
                      ? (dict.volunteersListing?.tryAdjusting || "Try adjusting your filters or search terms")
                      : (dict.volunteersListing?.checkBackLater || "Check back later for new candidates (impact agents)")
                  }
                  action={
                    (hasActiveFilters || searchQuery) ? (
