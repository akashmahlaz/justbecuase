...
export default async function AdminVolunteersPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground mb-2">{dict.admin?.volunteers?.title || "Manage Candidates"}</h1>
        <p className="text-muted-foreground">
          {dict.admin?.volunteers?.subtitle || "View and manage all candidate profiles"}
        </p>
      </div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{dict.admin?.volunteers?.totalImpactAgents || "Total Candidates"}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
      ...
      <VolunteersSearchableList 
        ...
        title={dict.admin?.volunteers?.allImpactAgents || "All Candidates"}
      />
    </div>
  )
}
