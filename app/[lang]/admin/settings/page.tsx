      ...
      <Tabs defaultValue="general" className="space-y-6">
        <TabsList className="flex flex-wrap h-auto gap-1">
          ...
          <TabsTrigger value="volunteer-plans" className="flex items-center gap-2">
            ...
            {dict.admin?.settings?.tabs?.volunteerPlans || "Candidate Plans"}
          </TabsTrigger>
            ...
            <CardContent className="space-y-4">
              <div className="bg-muted/50 p-4 rounded-lg space-y-3">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-medium">{dict.admin?.settings?.payment?.impactAgentProSubscription || "Candidate Pro Subscription"}</p>
                    <p className="text-sm text-muted-foreground">
                      {(dict.admin?.settings?.payment?.impactAgentProSubscriptionDescription || "Candidates with Pro subscription can apply to unlimited jobs. Free candidates are limited to {count} applications/month.").replace("{count}", String(settings?.volunteerFreeApplicationsPerMonth || 3))}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {dict.admin?.settings?.volunteerPlans?.freePlanTitle || "Candidate Free Plan Limits"}
              </CardTitle>
              <CardDescription>
                {dict.admin?.settings?.volunteerPlans?.freePlanDescription || "Configure limits for candidates on the free plan"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 flex items-center gap-4 pt-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {dict.admin?.settings?.volunteerPlans?.profileVisibilityHint || "Allow free plan candidates to be visible in search"}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {dict.admin?.settings?.volunteerPlans?.proPlanTitle || "Candidate Pro Plan"}
              </CardTitle>
              <CardDescription>
                {dict.admin?.settings?.volunteerPlans?.proPlanDescription || "Configure pricing and features for the candidate pro plan"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {dict.admin?.settings?.integrations?.requirePhoneVerificationHint || "Require candidates to verify their phone number during onboarding"}
                  </p>
                </div>
                <Switch
